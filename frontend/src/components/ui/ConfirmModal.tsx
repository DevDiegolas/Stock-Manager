  ...
  return (
    <div
      className={`fixed inset-0 z-[9999] flex items-center justify-center px-4 transition-all duration-200 ${
        visible ? 'bg-brand-night/40 backdrop-blur-sm' : 'bg-transparent'
      }`}
      onClick={() => close(onCancel)}
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className={`w-full max-w-sm rounded-3xl border border-white/60 bg-white/95 p-6 shadow-2xl backdrop-blur-xl transition-all duration-200 ${
          visible ? 'scale-100 opacity-100' : 'scale-95 opacity-0'
        }`}
      >
        <div className="flex flex-col items-center text-center">
          <div className={`flex h-14 w-14 items-center justify-center rounded-2xl ${v.bg}`}>
            {variant === 'danger' ? (
          ...
          <h3 className="mt-4 font-display text-xl font-bold text-brand-night">{title}</h3>
